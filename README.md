...
npm run ci                 # اجرای migrationها + build — دستور build مخصوص محیط‌های دیپلوی
...
ترکیب رایگان: **Vercel Hobby** (میزبانی) + **Neon** (Postgres) + **Vercel Blob** (فایل‌های رسانه).

> Vercel سرورلس است و فایل‌سیستم پایدار ندارد، پس کالکشن Media نمی‌تواند روی دیسک محلی بماند. پروژه این را خودکار تشخیص می‌دهد: اگر `BLOB_READ_WRITE_TOKEN` ست شده باشد رسانه‌ها روی Vercel Blob ذخیره می‌شوند، و اگر ست نشده باشد همان رفتار قبلی (پوشه‌ی محلی `media/`) برقرار است.
>
> توجه: پلن Hobby طبق شرایط استفاده‌ی Vercel برای مصارف **غیرتجاری** است؛ برای دمو و نمونه‌کار مناسب است، ولی برای سایت واقعیِ در حال کار باید به پلن Pro یا میزبان دیگری (مثلاً مسیر Docker/لیارا در همین ریپو) منتقل شوید.

**۱. ساخت پروژه در Vercel**
ریپو را از GitHub در Vercel ایمپورت کنید. دستور build از روی `vercel.json` خوانده می‌شود (`npm run ci`) که قبل از build، migrationها را روی پایگاه‌داده اجرا می‌کند — پس جدول‌ها قبل از اینکه build سراغ داده برود ساخته می‌شوند.

**۲. ساخت پایگاه‌داده Neon**
از تب **Storage** پروژه، گزینه‌ی Neon (Postgres) را از Marketplace بسازید و به پروژه وصل کنید. این کار متغیر `DATABASE_URL` را خودکار به پروژه اضافه می‌کند و پروژه همان را می‌خواند — نیازی به ست‌کردن دستی `DATABASE_URI` نیست.

> اگر اکانت Neon مستقلی دارید (خارج از Vercel)، به‌جای این کار رشته‌ی اتصال حالت **pooled** را دستی در متغیر `DATABASE_URI` بگذارید. پیکربندی هر دو نام را قبول می‌کند: `DATABASE_URI` ← `DATABASE_URL` ← `POSTGRES_URL`.

**۳. اتصال Vercel Blob**
از تب Storage پروژه یک Blob Store بسازید و به پروژه وصلش کنید. متغیر `BLOB_READ_WRITE_TOKEN` خودکار به تنظیمات پروژه اضافه می‌شود.

> ⚠️ هنگام ساخت، حتماً دسترسی store را **Public** بگذارید. پلاگین رسمی Payload فقط `access: 'public'` را پشتیبانی می‌کند و روی store خصوصی با خطای `Cannot use public access on a private store` شکست می‌خورد. store خصوصی هم به‌درد نمی‌خورد، چون تصاویر کاتالوگ باید بدون توکن امضاشده برای `next/image` قابل دسترسی باشند.

**۴. متغیرهای محیطی در Vercel**

| متغیر | مقدار |
|---|---|
| `PAYLOAD_SECRET` | **دستی** — یک رشته‌ی تصادفی و طولانی (ثابت بماند) |
| `DATABASE_URL` | خودکار توسط اینتگریشن Neon |
| `BLOB_READ_WRITE_TOKEN` | خودکار توسط Blob Store |
| `NEXT_PUBLIC_SITE_URL` | در دیپلوی اول ست نکنید |

تنها متغیری که باید دستی اضافه کنید `PAYLOAD_SECRET` است.

`NEXT_PUBLIC_SITE_URL` هنگام build داخل باندل نوشته می‌شود، و دامنه‌ی نهایی قبل از اولین دیپلوی هنوز معلوم نیست. اگر ست نشده باشد، پروژه خودکار از دامنه‌ی production پروژه‌ی Vercel استفاده می‌کند. بعد از وصل‌کردن دامنه‌ی اختصاصی، این متغیر را دستی ست کرده و یک بار Redeploy بزنید.

**۵. پر کردن سایت با داده‌ی نمونه**
چون رسانه‌ها روی Blob ذخیره می‌شوند، seed را می‌توانید از همین سیستم روی پایگاه‌داده‌ی production اجرا کنید:

```powershell
$env:DATABASE_URI="<neon-url>"; $env:PAYLOAD_SECRET="<secret>"; $env:BLOB_READ_WRITE_TOKEN="<token>"; $env:NODE_ENV="production"; npm run seed
```

⚠️ این دستور کل کاتالوگ (دسته‌بندی‌ها، برندها، محصولات، مقالات، رسانه‌ها) را **پاک** و از نو می‌سازد. کاربران ادمین دست‌نخورده می‌مانند.

`NODE_ENV=production` را حتماً بگذارید، وگرنه پیلود قبل از seed یک schema push کامل روی پایگاه‌داده‌ی production اجرا می‌کند (هم کند است و هم نباید در production اتفاق بیفتد).

اگر `BLOB_READ_WRITE_TOKEN` را جا بیندازید، اسکریپت بدون خطا تمام می‌شود ولی فایل‌ها روی دیسک همین سیستم می‌نشینند و آدرس‌های داخل پایگاه‌داده به فایل‌هایی اشاره می‌کنند که روی سرور وجود ندارند. برای تشخیص، مقدار `url` یک رکورد media را ببینید: اگر `/api/media/file/...` بود و توکن ست بود مشکلی نیست، ولی اگر seed بدون توکن اجرا شده باشد فایل روی Blob آپلود نشده — کافی است seed را دوباره با توکن اجرا کنید.

**۶. ساخت کاربر ادمین**
به `https://<your-domain>/admin` بروید و اولین کاربر ادمین را بسازید (اسکریپت seed کاربر نمی‌سازد).

⚠️ تا وقتی اولین کاربر ساخته نشده، این فرم برای **هر بازدیدکننده‌ای** باز است و هرکس می‌تواند ادمین سایت شود. بلافاصله بعد از اولین دیپلوی موفق این کار را انجام دهید. برای چک کردن وضعیت:

```bash
curl -s https://<your-domain>/api/users/init   # {"initialized":true} یعنی کاربر ساخته شده
```

**۷. یک Redeploy پایانی**
اگر seed را بعد از دیپلوی اول اجرا کردید، یک بار Redeploy بزنید تا صفحه‌های استاتیک محصولات و مقالات با داده‌ی واقعی ساخته شوند.

### به‌روزرسانی اسکیما بعد از دیپلوی

هر بار کالکشن یا گلوبالی را تغییر دادید، قبل از push یک migration بسازید:

```bash
npm run payload -- migrate:create <name>
```

فایل تولیدشده در `payload/migrations/` را کامیت کنید؛ Vercel در build بعدی خودش آن را اجرا می‌کند.

### رفع اشکال استقرار

**تصاویر لود نمی‌شوند و `/api/media/file/...` خطای ۵۰۰ می‌دهد**
یعنی اپ در زمان اجرا `BLOB_READ_WRITE_TOKEN` را نمی‌بیند، پس پلاگین خاموش می‌ماند و دنبال فایل روی دیسک محلی می‌گردد (که روی Vercel وجود ندارد). لاگ سرور در این حالت `File ... is missing on the disk` می‌دهد. چک کنید:

- متغیر باید در **Project → Settings → Environment Variables** باشد، نه فقط در صفحه‌ی خود Blob Store. آن صفحه توکن را چه وصل باشد چه نباشد نشان می‌دهد.
- تیک **Production** خورده باشد.
- مقدار بدون گیومه باشد.
- بعد از افزودن متغیر حتماً **Redeploy** بزنید؛ متغیرها در لحظه‌ی ساخت دیپلوی اسنپ‌شات می‌شوند و دیپلوی قدیمی‌تر آن‌ها را نمی‌بیند.

**آپلود عکس در پنل ادمین خطای ۵۰۰ با پیام `Something went wrong.` می‌دهد**
لاگ Runtime روی Vercel در این حالت `TypeError: ArrayBuffer: SharedArrayBuffer is not allowed.` نشان می‌دهد. `put()` در `@vercel/blob` بدنه‌ی فایل را از طریق `fetch` کتابخانه‌ی undici می‌فرستد، و undici هر بدنه‌ای که پشتش `SharedArrayBuffer` باشد را رد می‌کند؛ روی رانتایم سرورلس Vercel بافرهایی که Sharp برمی‌گرداند می‌توانند دقیقاً همین‌طور باشند (محلی این اتفاق نمی‌افتد، برای همین فقط در production دیده می‌شود).

هوک `payload/hooks/normalizeUploadBuffers.ts` این بافرها را درست قبل از آپلود در یک `Buffer` معمولی کپی می‌کند و روی کالکشن Media وصل است. اگر بعداً کالکشن آپلود جدیدی اضافه کردید، همین هوک را روی آن هم در `beforeChange` بگذارید وگرنه همین خطا برمی‌گردد.

**پنل ادمین سفید بالا می‌آید (صفحه ۲۰۰ می‌دهد ولی چیزی رندر نمی‌شود)**
یعنی `src/app/(payload)/admin/importMap.js` با پیکربندی فعلی هماهنگ نیست. هر بار پلاگینی به Payload اضافه یا حذف می‌کنید (یا کامپوننت سفارشی می‌سازید) این فایل باید دوباره تولید و کامیت شود:

```bash
npm run generate:importmap
```

**آدرس‌های sitemap و canonical دامنه‌ی اشتباه دارند**
`NEXT_PUBLIC_SITE_URL` در زمان build داخل باندل نوشته می‌شود. اگر دامنه‌ی پروژه بعد از build عوض شود (مثلاً rename پروژه یا افزودن دامنه‌ی اختصاصی)، مقدار قدیمی باقی می‌ماند. مقدار درست را صریح ست کنید و Redeploy بزنید.

### گزینه‌ی جایگزین: Docker / لیارا

`Dockerfile` و `liara.json` در ریپو برای میزبان‌های کانتینری با دیسک پایدار نگه داشته شده‌اند. در آن مسیر `BLOB_READ_WRITE_TOKEN` را ست **نکنید** تا رسانه‌ها روی دیسک بمانند، و مطمئن شوید یک دیسک پایدار روی `/app/media` مانت شده است. مقادیر `DATABASE_URI`، `PAYLOAD_SECRET` و `NEXT_PUBLIC_SITE_URL` باید به‌صورت `--build-arg` در زمان build پاس داده شوند.
